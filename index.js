...
(async () => {
  ...
  await page.goto("https://www.goodreads.com/choiceawards/best-books-2020",{timeout: 0});
  ...
  for (let i = 0; i < categoriesLinks.length; i++) {
    const books = [];
    await page.goto(categoriesLinks[i], {timeout: 0});
    ...
    for (let j = 0; j < booksLinks.length; j++) {
      await page.goto(booksLinks[j], {timeout: 0});
    ...
    results[i].books = filterBook;
  }


  //const number = Math.random();

  fs.writeFile("results.json", JSON.stringify(results[1]), function(err) {
    if (err) throw err;
